...
Generate the key straight into its own file, readable only by you:
...
Then create the `.env` file with just the Telegram placeholders:
...
The STEP 4 rule applies here too, and matters more on a rented server: the key
must not travel with the wallet file, and must not sit inside the deploy
directory.

First put the key on the VPS by itself — pasted in when prompted, not copied
alongside anything, and readable only by root:
...
Then create `.env` on the VPS with the Telegram values only, and copy up the
encrypted wallet file — that one file and nothing else:
...
wallet file, which is the pairing this layout exists to prevent.
...
The key prefix is required — PM2 captures the environment at registration, and
without it every scheduled run fails to decrypt the wallet file.
...
TO CHECK IN:
  ...
  • Balances: ENCRYPTION_KEY="$(cat ~/.airdrop-farm/enc.key)" npx tsx src/check-all-balances.ts
  • Activity: ENCRYPTION_KEY="$(cat ~/.airdrop-farm/enc.key)" npx tsx src/index.ts → option 5
